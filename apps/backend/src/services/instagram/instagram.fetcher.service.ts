...
export interface IgProfile {
  ...
  engagementRate: number | null; // percent
  recentMedia: IgMedia[];
}
...
export class InstagramFetcherService {
  ...
  async getProfile(orgId: string): Promise<IgProfile> {
    ...
    const recentMedia = await this.fetchRecentMedia(
      user.id,
    ...
    if (
      ...
      const totalInteractions = recentMedia.reduce(
        (sum, m) => sum + m.likeCount + m.commentsCount,
        0
      );
      const avgPerPost = totalInteractions / recentMedia.length;
      engagementRate =
        Math.round(
          (avgPerPost / user.instagramFollowers) * 100 * 100
        ) / 100;
    }
  ...
  private async fetchRecentMedia(
    ...
    try {
      ...
      if (!res.ok) {
        this.logger.warn(
          `IG media fetch failed (${res.status}): ${(await res.text()).slice(0, 200)}`
        );
        return [];
      }
